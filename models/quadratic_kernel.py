"""
Quadratic kernel implementation

This file is adapted from:
 - https://github.com/nmallinar/rfm-grokking/blob/main/models/quadratic_kernel.py
Minor changes made for compatibility (e.g., CUDA support).
Used with permission as part of a collaborative project.
"""
...
def quad_M_update(X, x, sol, P, batch_size=2,
    ...
    K = 3 * 2 * (X @ P @ x.T)**1
    a1 = sol.T.to(device)   
    X1 = (X @ P).reshape(X.shape[0], 1, X.shape[1]).to(device)
    a1 = a1.reshape(X.shape[0], -1, 1)
    
    step1 = a1 @ X1
    step1 = step1.reshape(-1, X1.shape[-1] * a1.shape[1])
    step2 = K.T @ step1
    G = step2.reshape(-1, a1.shape[1], X.shape[1])

    if centering:
        G_mean = torch.mean(G, axis=0, keepdim=True)
        G = G - G_mean

    bs = batch_size
    batches = torch.split(G, bs)
    for batch in batches:
        grad = batch.to(device)
        gradT = grad.transpose(1, 2)

        if diag_only:
            T = torch.sum(gradT * gradT, dim=-1)
            M += torch.sum(T, dim=0)
        else:
    ...
    if return_per_class_agop:
        for batch in batches:
            for class_i in range(G.shape[1]):
                g = batch[:, class_i]
                if len(per_class_agops) < G.shape[1]:
                    per_class_agops.append(g.T @ g)
                else:
                    per_class_agops[class_i] += g.T @ g
        for i in range(len(per_class_agops)):
            s = torch.linalg.svdvals(per_class_agops[i])
            U, _, Vt = torch.linalg.svd(per_class_agops[i])
            s = torch.sqrt(s)
            per_class_agops[i] = (U @ torch.diag(s) @ Vt)

    if diag_only:
        M = torch.diag(M)

    # Normalize eigenvalues (sqrtm-like)
    U, s, Vt = torch.linalg.svd(M)
    alpha = 0.5  # fixed for depth=2
    s = torch.pow(torch.abs(s), alpha)
    M = U @ torch.diag(s) @ Vt

    return M.to(device), per_class_agops