...
This file is adapted from:
 - https://github.com/nmallinar/rfm-grokking/blob/main/models/gaussian_kernel.py
...
def get_grads(X, sol, L, P, batch_size=2, K=None, centering=False, x=None,
    ...
    per_class_agops = []
    if return_per_class_agop:
        per_class_agops = [torch.zeros(d, d, device=device) for _ in range(G.shape[1])]
        for grad in batches:
            for class_i in range(G.shape[1]):
                gi = grad[:, class_i]
                per_class_agops[class_i] += gi.T @ gi
        for i in range(len(per_class_agops)):
            per_class_agops[i] /= len(G)
            eigvals, eigvecs = torch.linalg.eigh(per_class_agops[i])
            eigvals = torch.clamp(eigvals, min=0)
            per_class_agops[i] = eigvecs @ torch.diag(eigvals.sqrt()) @ eigvecs.T

    # Handle agop_power
    eigvals, eigvecs = torch.linalg.eigh(M)
    eigvals = torch.clamp(eigvals, min=0)
    if agop_power == 0.5:
        eigvals = eigvals.sqrt()
    elif isinstance(agop_power, int) and agop_power > 0:
        M = torch.linalg.matrix_power(M, agop_power)
        return M, per_class_agops
    else:
        eigvals = eigvals.pow(agop_power)
    M = eigvecs @ torch.diag(eigvals) @ eigvecs.T
